...
/* #1 */
/*
CREATE PROCEDURE SHOW_VENDOR_PRODUCTS
AS 
BEGIN
SELECT V.vend_id, V.vend_name, V.vend_country, STRING_AGG(CONVERT(NVARCHAR(MAX),TRIM(P.prod_name)),',') as products
FROM Vendors AS V 
FULL OUTER JOIN Products AS P ON V.vend_id = P.vend_id 
GROUP BY V.vend_id, V.vend_name, V.vend_country
END

EXEC SHOW_VENDOR_PRODUCTS
*/
/* #2 */
...
BEGIN 
	...
	(CASE
		WHEN cust_email IS NULL THEN CONCAT('kztoys.@',REPLACE(LOWER(cust_name),' ',''),'.com')
		ELSE cust_email
	END)
...
/* #3 */
/*
CREATE PROCEDURE REGULARITY_VISIT
AS
BEGIN
	SELECT C.cust_id, C.cust_name, C.cust_address, C.cust_city, C.cust_email, COUNT(O.order_num) AS amount_orders, 
	(CASE 
		WHEN COUNT(O.order_num) > 1 THEN 'regular customer'
		ELSE 'seldom customer'
	END) AS customer_description
	FROM Customers AS C
	JOIN Orders as O ON C.cust_id = O.cust_id 
	GROUP BY C.cust_id, C.cust_name, C.cust_address, C.cust_city, C.cust_email
END

EXEC REGULARITY_VISIT
*/

/* #4 */

/*
CREATE PROCEDURE COUNTRY_DESC
AS 
BEGIN
	...
	(CASE 
		...
		ELSE 'odd'
	END) AS vend_country_description FROM Vendors
END
...
AS
BEGIN
	DECLARE @full_cost FLOAT
	SET @full_cost = (SELECT (quantity*item_price) as full_cost FROM OrderItems
	WHERE order_num = @order_num AND prod_id = @prod_id)
	PRINT CONCAT('FULL COST ', @full_cost)
END

EXEC PROC_5 @PROD_ID = 'BR01', @ORDER_NUM = 20006	 
*/
...
SET @status = (CASE WHEN 
				(SELECT (SUM(CASE WHEN vend_address IS NULL THEN 1 ELSE 0 END) + 
						SUM(CASE WHEN vend_city IS NULL THEN 1 ELSE 0 END) +
						SUM(CASE WHEN vend_state IS NULL THEN 1 ELSE 0 END) +
						SUM(CASE WHEN vend_zip IS NULL THEN 1 ELSE 0 END) +
						SUM(CASE WHEN vend_country IS NULL THEN 1 ELSE 0 END)) as info
				FROM Vendors) < 1 THEN 'Information about location is filled'
				ELSE 'There are some vendors with unknown locations'
				END)
PRINT @status
END
...
/* #7 */
/*
CREATE PROCEDURE IS_THERE_VENDOR
AS 
BEGIN
SELECT C.cust_name, (CASE WHEN vend_state IS NULL THEN 'NO'
						  ELSE 'YES'
					END) AS VEND FROM Customers AS C
LEFT JOIN Vendors AS V ON C.cust_state = V.vend_state
END

EXEC IS_THERE_VENDOR
*/
/* #8 */

/*
CREATE PROCEDURE BETW_AVG
AS
BEGIN
  DECLARE @AVG FLOAT, @MAX FLOAT, @SEC_AVG FLOAT
  SET @AVG = (SELECT AVG(prod_price) FROM Products)
  SET @MAX = (SELECT MAX(prod_price) FROM Products)
  SET @SEC_AVG = (SELECT AVG(prod_price) FROM Products 
					WHERE prod_price BETWEEN @AVG AND @MAX)
  SELECT*FROM Products
  WHERE prod_price BETWEEN @AVG AND @SEC_AVG 
END
